...
class APIClient:
    ...
    def _process_api_response(self, content: str) -> str:
        ...
        # Look for standalone commentary paragraphs (common patterns)
        commentary_patterns = [
            r'\n\n\*[^*]+\*\s*
...
if __name__ == "__main__":
    main()
,  # Italic comments at end like "*This creates better flow*"
            r'\n\n\([^)]+\)\s*
        """Make API call to OpenRouter."""
        url = f"{self.config.get('api_base_url')}/chat/completions"
...
if __name__ == "__main__":
    main()
,  # Parenthetical comments at end like "(This improves readability)"
            r'\n\nNote:.*
        """Make API call to OpenRouter."""
        url = f"{self.config.get('api_base_url')}/chat/completions"
...
if __name__ == "__main__":
    main()
,    # Sections after horizontal rules
        ]
        ...
        for pattern in commentary_patterns:
            ...
            if match:
                commentary = match.group(0).strip()
                removed_sections.append(f"COMMENTARY: {commentary}")
                logging.info(f"Removed commentary section: {commentary[:50]}...")
                processed_content = processed_content[:match.start()].rstrip()
        
        # Save all removed content to a separate file
        if removed_sections:
            self._save_ai_commentary(removed_sections)
        
        # Log processing results
        if processed_content != original_content:
            chars_removed = len(original_content) - len(processed_content)
            logging.info(f"Total characters removed from AI response: {chars_removed}")
            logging.info(f"Removed {len(removed_sections)} commentary sections")
        
        return processed_content
    
    def _save_ai_commentary(self, removed_sections: list):
        """Save removed AI commentary to a separate file."""
        try:
            commentary_file = self.config.get('commentary_file', 'ai_commentary.txt')
            
            with open(commentary_file, 'a', encoding='utf-8') as f:
                f.write(f"\n{'='*60}\n")
                f.write(f"AI Commentary - {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"{'='*60}\n")
            ...
            logging.info(f"AI commentary saved to {commentary_file}")
            
        except Exception as e:
            logging.warning(f"Failed to save AI commentary to file: {e}")
            # Fall back to detailed logging
            logging.info("AI Commentary (file save failed):")
            for section in removed_sections:
